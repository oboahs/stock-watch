...
- 行情接口失败时可回退到 SQLite 本地缓存，并在报告中标注为非实时数据
...
- “报告/大模型”：维护日报保存目录、大模型分析开关和提示词。启用后，程序会要求大模型返回精简 JSON，浏览器日报优先展示结构化结论；如果调用失败，会在报告顶部显示失败原因。
- “邮件/推送/API”：维护大模型 API、SMTP 邮件、新闻 API Key、美股增强行情 API Key，以及后续 Telegram、企业微信、飞书接口参数。
- “日报预览”：查看 Markdown 原文，也可以点击“浏览器可视化”打开同名 HTML 报告。
